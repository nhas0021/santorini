...
if __name__ == "__main__":
    ...
    TILE_FLOOR = ImageTk.PhotoImage(Image.open(
        "./art/kenney_block-pack/PNG/Double (128px)/tileGrass.png").crop((0, 52, 128, 200)))

    TILE_LEVEL = (
        ImageTk.PhotoImage(Image.open(
            "./art/kenney_block-pack/PNG/Double (128px)/tileCastle_gate.png").crop((0, 52, 128, 200))),
        ImageTk.PhotoImage(Image.open(
            "./art/kenney_block-pack/PNG/Double (128px)/tileCastle.png").crop((0, 52, 128, 200))),
        ImageTk.PhotoImage(Image.open("./art/kenney_block-pack/PNG/Double (128px)/tileCastle_top.png").crop((0, 52, 128, 200))))

    TILE_LEVEL_OFFSETS = (-60, -120, -180, -180)

    # last level just replaces top with a top+spike
    TILE_LEVEL_FINAL = Image.open(
        "./art/kenney_block-pack/PNG/Double (128px)/tileCastle_topRoof.png").crop((0, 52, 128, 200))
    # endregion
    ...
    for row in range(GRID_SIZE):
        for col in range(GRID_SIZE):
            ...
            for i in range(map_state[col][row].height):
                canvas.create_image(local_pos[0],local_pos[1] + TILE_LEVEL_OFFSETS[i], image=TILE_LEVEL[i])
